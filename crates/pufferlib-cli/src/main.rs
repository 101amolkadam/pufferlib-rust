...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Train { env: _env, timesteps: _timesteps, lr: _lr, num_envs: _num_envs, policy: _policy } => {
            #[cfg(feature = "torch")]
            {
...
fn train(env_name: &str, timesteps: u64, lr: f64, num_envs: usize, policy_type: &str) -> Result<()> {
    use pufferlib::policy::{MlpPolicy, MlpConfig, LstmPolicy};
    use pufferlib::spaces::Space;
    use pufferlib::vector::{Serial, Parallel, VecEnv};
    
    tracing::info!(env = env_name, timesteps, lr, num_envs, policy = policy_type, "Starting training");
    ...
    match env_name {
        ...
        _ => {
            tracing::warn!("Environment '{}' not fully implemented for training yet", env_name);
        }
    }
...
fn eval(env_name: &str, episodes: usize) -> Result<()> {
    tracing::info!(env = env_name, episodes, "Starting evaluation (random policy)");
    ...
    match env_name {
        ...
        "bandit" => {
            ...
            for step in 0..steps {
                ...
                let result = env.step(&action);
                println!("Step {}: action={}, reward={}", step, step % 4, result.reward);
            }
        }
        _ => {
            tracing::error!("Unknown environment: {}", env_name);
