...
impl LstmPolicy {
    ...
    pub fn new(
        ...
        // Simple encoder
        let encoder = nn::seq()
            .add(nn::linear(&root / "encoder", obs_size, hidden_size, Default::default()))
            .add_fn(|x| x.relu());
        ...
        // Output heads
        let actor = nn::linear(&root / "actor", hidden_size, num_actions, Default::default());
    ...
    pub fn forward_with_state(
        ...
        let (h0, c0) = hidden_state.unwrap_or_else(|| {
            let h = Tensor::zeros([1, batch_size, self.hidden_size], (Kind::Float, self.device));
            let c = Tensor::zeros([1, batch_size, self.hidden_size], (Kind::Float, self.device));
            (h, c)
        });
    ...
    pub fn num_parameters(&self) -> usize {
        self.vs.trainable_variables().iter().map(|t| t.numel()).sum()
    }
}
