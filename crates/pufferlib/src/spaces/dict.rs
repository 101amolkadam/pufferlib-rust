...
impl Dict {
    ...
    pub fn from_pairs(pairs: Vec<(&str, DynSpace)>) -> Self {
        let spaces: HashMap<_, _> = pairs.into_iter().map(|(k, v)| (k.to_string(), v)).collect();
        Self::new(spaces)
    }
...
mod tests {
    ...
    fn test_dict_creation() {
        let dict = Dict::from_pairs(vec![
            (
                "position",
                DynSpace::Box(BoxSpace::uniform(&[3], -1.0, 1.0)),
            ),
    ...
    fn test_dict_contains() {
        let dict = Dict::from_pairs(vec![("pos", DynSpace::Discrete(Discrete::new(2)))]);

        let mut valid = HashMap::new();
        valid.insert(
            "pos".to_string(),
            ndarray::ArrayD::from_elem(ndarray::IxDyn(&[1]), 0.0),
        );
        ...
        invalid_val.insert(
            "pos".to_string(),
            ndarray::ArrayD::from_elem(ndarray::IxDyn(&[1]), 2.0),
        );
