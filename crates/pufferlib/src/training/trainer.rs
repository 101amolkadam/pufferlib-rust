...
use crate::policy::{Policy, HasVarStore};
...
impl<P: Policy + HasVarStore, V: VecEnvBackend> Trainer<P, V> {
    ...
    pub fn train(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        while self.global_step < self.config.total_timesteps {
            ...
            let last_obs = self.buffer.observations.narrow(0, total_steps - self.num_envs as i64, self.num_envs as i64)
    ...
    fn collect_rollout(&mut self, initial_obs: &ndarray::Array2<f32>) {
        ...
        for _ in 0..steps_per_env {
            ...
            let action_vec: Vec<f32> = Vec::try_from(action.to_device(Device::Cpu).flatten(0, -1)).unwrap();
            ...
            let dones: Vec<f32> = dones_bool.iter().map(|&d| if d { 1.0 } else { 0.0 }).collect();
            ...
            if let Some(states) = next_state {
                let dones_dev = Tensor::from_slice(&dones.iter().map(|&d| if d > 0.5 { 0.0 } else { 1.0 }).collect::<Vec<f32>>())
                    .to_device(self.config.device)
